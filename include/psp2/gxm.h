...
typedef struct SceGxmInitializeParams {
	unsigned int flags;                               //!< One or more ::SceGxmInitializeFlags.
...
typedef enum SceGxmMemoryAttribFlags {
	SCE_GXM_MEMORY_ATTRIB_READ  = 1, //!< Memory region readable by the GPU.
	SCE_GXM_MEMORY_ATTRIB_WRITE = 2, //!< Memory region writeable by the GPU.
	SCE_GXM_MEMORY_ATTRIB_RW    = (SCE_GXM_MEMORY_ATTRIB_READ | SCE_GXM_MEMORY_ATTRIB_WRITE) //!< Memory region both readable and writeable by the GPU.
...
typedef enum SceGxmAttributeFormat {
	...
	SCE_GXM_ATTRIBUTE_FORMAT_U8N,    //!< 8-bit normalized unsigned integer.
	SCE_GXM_ATTRIBUTE_FORMAT_S8N,    //!< 8-bit normalized signed integer.
	SCE_GXM_ATTRIBUTE_FORMAT_U16N,   //!< 16-bit normalized unsigned integer.
	SCE_GXM_ATTRIBUTE_FORMAT_S16N,   //!< 16-bit normalized signed integer.
	SCE_GXM_ATTRIBUTE_FORMAT_F16,    //!< 16-bit half-float.
	SCE_GXM_ATTRIBUTE_FORMAT_F32,    //!< 32-bit float.
	SCE_GXM_ATTRIBUTE_FORMAT_UNTYPED //!< Typeless.
} SceGxmAttributeFormat;

/** Depth/stencil surface formats. */
typedef enum SceGxmDepthStencilFormat {
	SCE_GXM_DEPTH_STENCIL_FORMAT_DF32     = 0x00044000u, //!< 32-bit floating point depth surface.
	SCE_GXM_DEPTH_STENCIL_FORMAT_S8       = 0x00022000u, //!< 8-bit integer stencil surface.
	SCE_GXM_DEPTH_STENCIL_FORMAT_DF32_S8  = 0x00066000u, //!< 32-bit floating point depth surface and 8-bit integer stencil surface.
	SCE_GXM_DEPTH_STENCIL_FORMAT_DF32M    = 0x000CC000U, //!< 32-bit floating point depth surface with one bit reserved for mask update.
	SCE_GXM_DEPTH_STENCIL_FORMAT_DF32M_S8 = 0x000EE000U, //!< 32-bit floating point depth surface with one bit reserved for mask update and 8-bit integer stencil surface.
	SCE_GXM_DEPTH_STENCIL_FORMAT_S8D24    = 0x01266000u, //!< Packed 8-bit integer stencil and 24-bit integer depth surfaces.
	SCE_GXM_DEPTH_STENCIL_FORMAT_D16      = 0x02444000u  //!< 16-bit integer depth surface.
} SceGxmDepthStencilFormat;

/** Draw primitives. */
typedef enum SceGxmPrimitiveType {
	SCE_GXM_PRIMITIVE_TRIANGLES       = 0x00000000u, //!< Triangles primitive.
	SCE_GXM_PRIMITIVE_LINES           = 0x04000000u, //!< Lines primitive.
	SCE_GXM_PRIMITIVE_POINTS          = 0x08000000u, //!< Points primitive.
	SCE_GXM_PRIMITIVE_TRIANGLE_STRIP  = 0x0C000000u, //!< Triangle strips primitive.
	SCE_GXM_PRIMITIVE_TRIANGLE_FAN    = 0x10000000u, //!< Triangle fans primitive.
	SCE_GXM_PRIMITIVE_TRIANGLE_EDGES  = 0x14000000u  //!< Triangle edges primitive.
} SceGxmPrimitiveType;
...
typedef enum SceGxmRegionClipMode {
	SCE_GXM_REGION_CLIP_NONE    = 0x00000000u, //!< Disable tiles clipping.
	SCE_GXM_REGION_CLIP_ALL     = 0x40000000u, //!< Clip tiles inside and outside the region.
	SCE_GXM_REGION_CLIP_OUTSIDE = 0x80000000u, //!< Clip tiles inside the region.
	SCE_GXM_REGION_CLIP_INSIDE  = 0xC0000000u  //!< Clip tiles outside the region.
} SceGxmRegionClipMode;

/** Depth test functions. */
typedef enum SceGxmDepthFunc {
	SCE_GXM_DEPTH_FUNC_NEVER          = 0x00000000u, //!< Depth test never passes.
	SCE_GXM_DEPTH_FUNC_LESS           = 0x00400000u, //!< Depth test passes when fragment depth is less than the current stored value.
	SCE_GXM_DEPTH_FUNC_EQUAL          = 0x00800000u, //!< Depth test passes when fragment depth is equal to the current stored value.
	SCE_GXM_DEPTH_FUNC_LESS_EQUAL     = 0x00C00000u, //!< Depth test passes when fragment depth is less or equal than the current stored value.
	SCE_GXM_DEPTH_FUNC_GREATER        = 0x01000000u, //!< Depth test passes when fragment depth is greater than the current stored value.
	SCE_GXM_DEPTH_FUNC_NOT_EQUAL      = 0x01400000u, //!< Depth test passes when fragment depth is not equal to the current stored value.
	SCE_GXM_DEPTH_FUNC_GREATER_EQUAL  = 0x01800000u, //!< Depth test passes when fragment depth is greater or equal than the current stored value.
	SCE_GXM_DEPTH_FUNC_ALWAYS         = 0x01C00000u  //!< Depth test always passes.
} SceGxmDepthFunc;

/** Stencil test functions. */
typedef enum SceGxmStencilFunc {
	SCE_GXM_STENCIL_FUNC_NEVER          = 0x00000000u, //!< Stencil test never passes.
	SCE_GXM_STENCIL_FUNC_LESS           = 0x02000000u, //!< Stencil test passes when fragment stencil value is less than the current stored value.
	SCE_GXM_STENCIL_FUNC_EQUAL          = 0x04000000u, //!< Stencil test passes when fragment stencil value is equal to the current stored value.
	SCE_GXM_STENCIL_FUNC_LESS_EQUAL     = 0x06000000u, //!< Stencil test passes when fragment stencil value is less or equal than the current stored value.
	SCE_GXM_STENCIL_FUNC_GREATER        = 0x08000000u, //!< Stencil test passes when fragment stencil value is greater than the current stored value.
	SCE_GXM_STENCIL_FUNC_NOT_EQUAL      = 0x0A000000u, //!< Stencil test passes when fragment stencil value is not equal to the current stored value.
	SCE_GXM_STENCIL_FUNC_GREATER_EQUAL  = 0x0C000000u, //!< Stencil test passes when fragment stencil value is greater or equal than the current stored value.
	SCE_GXM_STENCIL_FUNC_ALWAYS         = 0x0E000000u  //!< Stencil test always passes.
} SceGxmStencilFunc;

/** Stencil test operations. */
typedef enum SceGxmStencilOp {
	SCE_GXM_STENCIL_OP_KEEP      = 0x00000000u, //!< Keep the current stored value.
	SCE_GXM_STENCIL_OP_ZERO      = 0x00000001u, //!< Set the current stored value to 0.
	SCE_GXM_STENCIL_OP_REPLACE   = 0x00000002u, //!< Replace the current stored value with the fragment value.
	SCE_GXM_STENCIL_OP_INCR      = 0x00000003u, //!< Increment the current stored value by 1.
	SCE_GXM_STENCIL_OP_DECR      = 0x00000004u, //!< Decrement the current stored value by 1.
	SCE_GXM_STENCIL_OP_INVERT    = 0x00000005u, //!< Bitwise flip the current stored value.
	SCE_GXM_STENCIL_OP_INCR_WRAP = 0x00000006u, //!< Increment the current stored value by 1 with wrapping in the 0-255 range.
	SCE_GXM_STENCIL_OP_DECR_WRAP = 0x00000007u  //!< Decrement the current stored value by 1 with wrapping in the 0-255 range.
} SceGxmStencilOp;

/** Hardware culling modes. */
typedef enum SceGxmCullMode {
	SCE_GXM_CULL_NONE = 0x00000000u, //!< Disable hardware culling.
	SCE_GXM_CULL_CW   = 0x00000001u, //!< Clockwise hardware culling.
	SCE_GXM_CULL_CCW  = 0x00000002u  //!< Counter-clockwise hardware culling.
} SceGxmCullMode;
...
typedef enum SceGxmColorFormat {
	...
	SCE_GXM_COLOR_FORMAT_U8U8U8U8_ABGR = SCE_GXM_COLOR_BASE_FORMAT_U8U8U8U8 | SCE_GXM_COLOR_SWIZZLE4_ABGR, //!< 32-bit unsigned ABGR color format.
	SCE_GXM_COLOR_FORMAT_U8U8U8U8_ARGB = SCE_GXM_COLOR_BASE_FORMAT_U8U8U8U8 | SCE_GXM_COLOR_SWIZZLE4_ARGB, //!< 32-bit unsigned ARGB color format.
	SCE_GXM_COLOR_FORMAT_U8U8U8U8_RGBA = SCE_GXM_COLOR_BASE_FORMAT_U8U8U8U8 | SCE_GXM_COLOR_SWIZZLE4_RGBA, //!< 32-bit unsigned RGBA color format.
	SCE_GXM_COLOR_FORMAT_U8U8U8U8_BGRA = SCE_GXM_COLOR_BASE_FORMAT_U8U8U8U8 | SCE_GXM_COLOR_SWIZZLE4_BGRA, //!< 32-bit unsigned BGRA color format.

	SCE_GXM_COLOR_FORMAT_U8U8U8_BGR = SCE_GXM_COLOR_BASE_FORMAT_U8U8U8 | SCE_GXM_COLOR_SWIZZLE3_BGR, //!< 24-bit unsigned BGR color format.
	SCE_GXM_COLOR_FORMAT_U8U8U8_RGB = SCE_GXM_COLOR_BASE_FORMAT_U8U8U8 | SCE_GXM_COLOR_SWIZZLE3_RGB, //!< 24-bit unsigned RGB color format.

	SCE_GXM_COLOR_FORMAT_U5U6U5_BGR = SCE_GXM_COLOR_BASE_FORMAT_U5U6U5 | SCE_GXM_COLOR_SWIZZLE3_BGR, //!< 16-bit unsigned BGR565 color format.
	SCE_GXM_COLOR_FORMAT_U5U6U5_RGB = SCE_GXM_COLOR_BASE_FORMAT_U5U6U5 | SCE_GXM_COLOR_SWIZZLE3_RGB, //!< 16-bit unsigned RGB565 color format.

	SCE_GXM_COLOR_FORMAT_U1U5U5U5_ABGR = SCE_GXM_COLOR_BASE_FORMAT_U1U5U5U5 | SCE_GXM_COLOR_SWIZZLE4_ABGR, //!< 16-bit unsigned ABGR1555 color format.
	SCE_GXM_COLOR_FORMAT_U1U5U5U5_ARGB = SCE_GXM_COLOR_BASE_FORMAT_U1U5U5U5 | SCE_GXM_COLOR_SWIZZLE4_ARGB, //!< 16-bit unsigned ARGB1555 color format.
	SCE_GXM_COLOR_FORMAT_U5U5U5U1_RGBA = SCE_GXM_COLOR_BASE_FORMAT_U1U5U5U5 | SCE_GXM_COLOR_SWIZZLE4_RGBA, //!< 16-bit unsigned RGBA5551 color format.
	SCE_GXM_COLOR_FORMAT_U5U5U5U1_BGRA = SCE_GXM_COLOR_BASE_FORMAT_U1U5U5U5 | SCE_GXM_COLOR_SWIZZLE4_BGRA, //!< 16-bit unsigned BGRA5551 color format.

	SCE_GXM_COLOR_FORMAT_U4U4U4U4_ABGR = SCE_GXM_COLOR_BASE_FORMAT_U4U4U4U4 | SCE_GXM_COLOR_SWIZZLE4_ABGR, //!< 16-bit unsigned ABGR4444 color format.
	SCE_GXM_COLOR_FORMAT_U4U4U4U4_ARGB = SCE_GXM_COLOR_BASE_FORMAT_U4U4U4U4 | SCE_GXM_COLOR_SWIZZLE4_ARGB, //!< 16-bit unsigned ARGB4444 color format.
	SCE_GXM_COLOR_FORMAT_U4U4U4U4_RGBA = SCE_GXM_COLOR_BASE_FORMAT_U4U4U4U4 | SCE_GXM_COLOR_SWIZZLE4_RGBA, //!< 16-bit unsigned RGBA4444 color format.
	SCE_GXM_COLOR_FORMAT_U4U4U4U4_BGRA = SCE_GXM_COLOR_BASE_FORMAT_U4U4U4U4 | SCE_GXM_COLOR_SWIZZLE4_BGRA, //!< 16-bit unsigned BGRA4444 color format.

	SCE_GXM_COLOR_FORMAT_U8U3U3U2_ARGB = SCE_GXM_COLOR_BASE_FORMAT_U8U3U3U2, //!< 16-bit unsigned ARGB8332 color format.

	SCE_GXM_COLOR_FORMAT_F16_R = SCE_GXM_COLOR_BASE_FORMAT_F16 | SCE_GXM_COLOR_SWIZZLE1_R, //!< 16-bit half-float R color format.
	SCE_GXM_COLOR_FORMAT_F16_G = SCE_GXM_COLOR_BASE_FORMAT_F16 | SCE_GXM_COLOR_SWIZZLE1_G, //!< 16-bit half-float G color format.

	SCE_GXM_COLOR_FORMAT_F16F16_GR = SCE_GXM_COLOR_BASE_FORMAT_F16F16 | SCE_GXM_COLOR_SWIZZLE2_GR, //!< 32-bit half-float GR color format.
	SCE_GXM_COLOR_FORMAT_F16F16_RG = SCE_GXM_COLOR_BASE_FORMAT_F16F16 | SCE_GXM_COLOR_SWIZZLE2_RG, //!< 32-bit half-float RG color format.

	SCE_GXM_COLOR_FORMAT_F32_R = SCE_GXM_COLOR_BASE_FORMAT_F32 | SCE_GXM_COLOR_SWIZZLE1_R, //!< 32-bit float R color format.

	SCE_GXM_COLOR_FORMAT_S16_R = SCE_GXM_COLOR_BASE_FORMAT_S16 | SCE_GXM_COLOR_SWIZZLE1_R, //!< 16-bit signed R color format.
	SCE_GXM_COLOR_FORMAT_S16_G = SCE_GXM_COLOR_BASE_FORMAT_S16 | SCE_GXM_COLOR_SWIZZLE1_G, //!< 16-bit signed G color format.

	SCE_GXM_COLOR_FORMAT_S16S16_GR = SCE_GXM_COLOR_BASE_FORMAT_S16S16 | SCE_GXM_COLOR_SWIZZLE2_GR, //!< 32-bit signed G16R16 color format.
	SCE_GXM_COLOR_FORMAT_S16S16_RG = SCE_GXM_COLOR_BASE_FORMAT_S16S16 | SCE_GXM_COLOR_SWIZZLE2_RG, //!< 32-bit signed R16G16 color format.

	SCE_GXM_COLOR_FORMAT_U16_R = SCE_GXM_COLOR_BASE_FORMAT_U16 | SCE_GXM_COLOR_SWIZZLE1_R, //!< 16-bit unsigned R16 color format.
	SCE_GXM_COLOR_FORMAT_U16_G = SCE_GXM_COLOR_BASE_FORMAT_U16 | SCE_GXM_COLOR_SWIZZLE1_G, //!< 16-bit unsigned G16 color format.

	SCE_GXM_COLOR_FORMAT_U16U16_GR = SCE_GXM_COLOR_BASE_FORMAT_U16U16 | SCE_GXM_COLOR_SWIZZLE2_GR, //!< 32-bit unsigned G16R16 color format.
	SCE_GXM_COLOR_FORMAT_U16U16_RG = SCE_GXM_COLOR_BASE_FORMAT_U16U16 | SCE_GXM_COLOR_SWIZZLE2_RG, //!< 32-bit unsigned R16G16 color format.

	SCE_GXM_COLOR_FORMAT_U2U10U10U10_ABGR = SCE_GXM_COLOR_BASE_FORMAT_U2U10U10U10 | SCE_GXM_COLOR_SWIZZLE4_ABGR, //!< 32-bit unsigned A2B10G10R10 color format.
	SCE_GXM_COLOR_FORMAT_U2U10U10U10_ARGB = SCE_GXM_COLOR_BASE_FORMAT_U2U10U10U10 | SCE_GXM_COLOR_SWIZZLE4_ARGB, //!< 32-bit unsigned A2R10G10B10 color format.
	SCE_GXM_COLOR_FORMAT_U10U10U10U2_RGBA = SCE_GXM_COLOR_BASE_FORMAT_U2U10U10U10 | SCE_GXM_COLOR_SWIZZLE4_RGBA, //!< 32-bit unsigned R10G10B10A2 color format.
	SCE_GXM_COLOR_FORMAT_U10U10U10U2_BGRA = SCE_GXM_COLOR_BASE_FORMAT_U2U10U10U10 | SCE_GXM_COLOR_SWIZZLE4_BGRA, //!< 32-bit unsigned B10G10R10A2 color format.

	SCE_GXM_COLOR_FORMAT_U8_R = SCE_GXM_COLOR_BASE_FORMAT_U8 | SCE_GXM_COLOR_SWIZZLE1_R, //!< 8-bit unsigned R color format.
	SCE_GXM_COLOR_FORMAT_U8_A = SCE_GXM_COLOR_BASE_FORMAT_U8 | SCE_GXM_COLOR_SWIZZLE1_A, //!< 8-bit unsigned A color format.

	SCE_GXM_COLOR_FORMAT_S8_R = SCE_GXM_COLOR_BASE_FORMAT_S8 | SCE_GXM_COLOR_SWIZZLE1_R, //!< 8-bit signed R color format.
	SCE_GXM_COLOR_FORMAT_S8_A = SCE_GXM_COLOR_BASE_FORMAT_S8 | SCE_GXM_COLOR_SWIZZLE1_A, //!< 8-bit signed A color format.

	SCE_GXM_COLOR_FORMAT_U6S5S5_BGR = SCE_GXM_COLOR_BASE_FORMAT_S5S5U6 | SCE_GXM_COLOR_SWIZZLE3_BGR, //!< 16-bit signed BGR556 with unsigned blue channel color format.
	SCE_GXM_COLOR_FORMAT_S5S5U6_RGB = SCE_GXM_COLOR_BASE_FORMAT_S5S5U6 | SCE_GXM_COLOR_SWIZZLE3_RGB, //!< 16-bit signed RGB655 with unsigned blue channel color format.

	SCE_GXM_COLOR_FORMAT_U8U8_GR = SCE_GXM_COLOR_BASE_FORMAT_U8U8 | SCE_GXM_COLOR_SWIZZLE2_GR, //!< 16-bit unsigned GR88 color format.
	SCE_GXM_COLOR_FORMAT_U8U8_RG = SCE_GXM_COLOR_BASE_FORMAT_U8U8 | SCE_GXM_COLOR_SWIZZLE2_RG, //!< 16-bit unsigned RG88 color format.
	SCE_GXM_COLOR_FORMAT_U8U8_RA = SCE_GXM_COLOR_BASE_FORMAT_U8U8 | SCE_GXM_COLOR_SWIZZLE2_RA, //!< 16-bit unsigned RA88 color format.
	SCE_GXM_COLOR_FORMAT_U8U8_AR = SCE_GXM_COLOR_BASE_FORMAT_U8U8 | SCE_GXM_COLOR_SWIZZLE2_AR, //!< 16-bit unsigned AR88 color format.

	SCE_GXM_COLOR_FORMAT_S8S8_GR = SCE_GXM_COLOR_BASE_FORMAT_S8S8 | SCE_GXM_COLOR_SWIZZLE2_GR, //!< 16-bit signed GR88 color format.
	SCE_GXM_COLOR_FORMAT_S8S8_RG = SCE_GXM_COLOR_BASE_FORMAT_S8S8 | SCE_GXM_COLOR_SWIZZLE2_RG, //!< 16-bit signed RG88 color format.
	SCE_GXM_COLOR_FORMAT_S8S8_RA = SCE_GXM_COLOR_BASE_FORMAT_S8S8 | SCE_GXM_COLOR_SWIZZLE2_RA, //!< 16-bit signed RA88 color format.
	SCE_GXM_COLOR_FORMAT_S8S8_AR = SCE_GXM_COLOR_BASE_FORMAT_S8S8 | SCE_GXM_COLOR_SWIZZLE2_AR, //!< 16-bit signed AR88 color format.

	SCE_GXM_COLOR_FORMAT_U8S8S8U8_ABGR = SCE_GXM_COLOR_BASE_FORMAT_U8S8S8U8 | SCE_GXM_COLOR_SWIZZLE4_ABGR, //!< 32-bit unsigned ABGR8888 with signed blue and green channels color format.
	SCE_GXM_COLOR_FORMAT_U8U8S8S8_ARGB = SCE_GXM_COLOR_BASE_FORMAT_U8S8S8U8 | SCE_GXM_COLOR_SWIZZLE4_ARGB, //!< 32-bit unsigned ARGB8888 with signed blue and green channels color format.
	SCE_GXM_COLOR_FORMAT_U8S8S8U8_RGBA = SCE_GXM_COLOR_BASE_FORMAT_U8S8S8U8 | SCE_GXM_COLOR_SWIZZLE4_RGBA, //!< 32-bit unsigned RGBA8888 with signed blue and green channels color format.
	SCE_GXM_COLOR_FORMAT_S8S8U8U8_BGRA = SCE_GXM_COLOR_BASE_FORMAT_U8S8S8U8 | SCE_GXM_COLOR_SWIZZLE4_BGRA, //!< 32-bit unsigned BGRA8888 with signed blue and green channels color format.

	SCE_GXM_COLOR_FORMAT_S8S8S8S8_ABGR = SCE_GXM_COLOR_BASE_FORMAT_S8S8S8S8 | SCE_GXM_COLOR_SWIZZLE4_ABGR, //!< 32-bit signed ABGR8888 color format.
	SCE_GXM_COLOR_FORMAT_S8S8S8S8_ARGB = SCE_GXM_COLOR_BASE_FORMAT_S8S8S8S8 | SCE_GXM_COLOR_SWIZZLE4_ARGB, //!< 32-bit signed ARGB8888 color format.
	SCE_GXM_COLOR_FORMAT_S8S8S8S8_RGBA = SCE_GXM_COLOR_BASE_FORMAT_S8S8S8S8 | SCE_GXM_COLOR_SWIZZLE4_RGBA, //!< 32-bit signed RGBA8888 color format.
	SCE_GXM_COLOR_FORMAT_S8S8S8S8_BGRA = SCE_GXM_COLOR_BASE_FORMAT_S8S8S8S8 | SCE_GXM_COLOR_SWIZZLE4_BGRA, //!< 32-bit signed BGRA8888 color format.

	SCE_GXM_COLOR_FORMAT_F16F16F16F16_ABGR = SCE_GXM_COLOR_BASE_FORMAT_F16F16F16F16 | SCE_GXM_COLOR_SWIZZLE4_ABGR, //!< 64-bit half-float ABGR color format.
	SCE_GXM_COLOR_FORMAT_F16F16F16F16_ARGB = SCE_GXM_COLOR_BASE_FORMAT_F16F16F16F16 | SCE_GXM_COLOR_SWIZZLE4_ARGB, //!< 64-bit half-float ARGB color format.
	SCE_GXM_COLOR_FORMAT_F16F16F16F16_RGBA = SCE_GXM_COLOR_BASE_FORMAT_F16F16F16F16 | SCE_GXM_COLOR_SWIZZLE4_RGBA, //!< 64-bit half-float RGBA color format.
	SCE_GXM_COLOR_FORMAT_F16F16F16F16_BGRA = SCE_GXM_COLOR_BASE_FORMAT_F16F16F16F16 | SCE_GXM_COLOR_SWIZZLE4_BGRA, //!< 64-bit half-float BGRA color format.

	SCE_GXM_COLOR_FORMAT_F32F32_GR = SCE_GXM_COLOR_BASE_FORMAT_F32F32 | SCE_GXM_COLOR_SWIZZLE2_GR, //!< 64-bit float GR color format.
	SCE_GXM_COLOR_FORMAT_F32F32_RG = SCE_GXM_COLOR_BASE_FORMAT_F32F32 | SCE_GXM_COLOR_SWIZZLE2_RG, //!< 64-bit float RG color format.

	SCE_GXM_COLOR_FORMAT_F10F11F11_BGR = SCE_GXM_COLOR_BASE_FORMAT_F11F11F10 | SCE_GXM_COLOR_SWIZZLE3_BGR, //!< 32-bit packed floating point B10G11R11 color format.
	SCE_GXM_COLOR_FORMAT_F11F11F10_RGB = SCE_GXM_COLOR_BASE_FORMAT_F11F11F10 | SCE_GXM_COLOR_SWIZZLE3_RGB, //!< 32-bit packed floating point R11G11B10 color format.

	SCE_GXM_COLOR_FORMAT_SE5M9M9M9_BGR = SCE_GXM_COLOR_BASE_FORMAT_SE5M9M9M9 | SCE_GXM_COLOR_SWIZZLE3_BGR, //!< 32-bit packed floats with 5-bit shared exponent and 9-bit mantissa BGR color format.
	SCE_GXM_COLOR_FORMAT_SE5M9M9M9_RGB = SCE_GXM_COLOR_BASE_FORMAT_SE5M9M9M9 | SCE_GXM_COLOR_SWIZZLE3_RGB, //!< 32-bit packed floats with 5-bit shared exponent and 9-bit mantissa RGB color format.

	SCE_GXM_COLOR_FORMAT_U2F10F10F10_ABGR = SCE_GXM_COLOR_BASE_FORMAT_U2F10F10F10 | SCE_GXM_COLOR_SWIZZLE4_ABGR, //!< 32-bit packed 2-bit unsigned integer A and 10-bit BGR floats color format.
	SCE_GXM_COLOR_FORMAT_U2F10F10F10_ARGB = SCE_GXM_COLOR_BASE_FORMAT_U2F10F10F10 | SCE_GXM_COLOR_SWIZZLE4_ARGB, //!< 32-bit packed 2-bit unsigned integer A and 10-bit RGB floats color format.
	SCE_GXM_COLOR_FORMAT_F10F10F10U2_RGBA = SCE_GXM_COLOR_BASE_FORMAT_U2F10F10F10 | SCE_GXM_COLOR_SWIZZLE4_RGBA, //!< 32-bit packed 10-bit RGB floats and 2-bit unsigned integer A color format.
	SCE_GXM_COLOR_FORMAT_F10F10F10U2_BGRA = SCE_GXM_COLOR_BASE_FORMAT_U2F10F10F10 | SCE_GXM_COLOR_SWIZZLE4_BGRA, //!< 32-bit packed 10-bit BGR floats and 2-bit unsigned integer A color format.

	SCE_GXM_COLOR_FORMAT_A8B8G8R8 = SCE_GXM_COLOR_FORMAT_U8U8U8U8_ABGR, //!< 32-bit unsigned ABGR color format (Legacy naming).
	SCE_GXM_COLOR_FORMAT_A8R8G8B8 = SCE_GXM_COLOR_FORMAT_U8U8U8U8_ARGB, //!< 32-bit unsigned ARGB color format (Legacy naming).
	SCE_GXM_COLOR_FORMAT_R5G6B5   = SCE_GXM_COLOR_FORMAT_U5U6U5_RGB, //!< 16-bit unsigned RGB565 color format (Legacy naming).
	SCE_GXM_COLOR_FORMAT_A1R5G5B5 = SCE_GXM_COLOR_FORMAT_U1U5U5U5_ARGB, //!< 16-bit unsigned ARGB1555 color format (Legacy naming).
	SCE_GXM_COLOR_FORMAT_A4R4G4B4 = SCE_GXM_COLOR_FORMAT_U4U4U4U4_ARGB, //!< 16-bit unsigned ARGB4444 color format (Legacy naming).
	SCE_GXM_COLOR_FORMAT_A8 = SCE_GXM_COLOR_FORMAT_U8_A //!< 8-bit unsigned A color format (Legacy naming).
} SceGxmColorFormat;

/** Color surfaces memory layouts. */
typedef enum SceGxmColorSurfaceType {
	SCE_GXM_COLOR_SURFACE_LINEAR    = 0x00000000u, //!< Linear memory layout.
	SCE_GXM_COLOR_SURFACE_TILED     = 0x04000000u, //!< Tiled memory layout.
	SCE_GXM_COLOR_SURFACE_SWIZZLED  = 0x08000000u  //!< Swizzled memory layout.
...
typedef enum SceGxmColorSurfaceDitherMode {
	SCE_GXM_COLOR_SURFACE_DITHER_DISABLED   = 0x00000000u, //!< Dithering disabled.
	SCE_GXM_COLOR_SURFACE_DITHER_ENABLED    = 0x00000008u  //!< Dithering enabled.
} SceGxmColorSurfaceDitherMode;

/** Depth/stencil surface memory layouts. */
typedef enum SceGxmDepthStencilSurfaceType {
	SCE_GXM_DEPTH_STENCIL_SURFACE_LINEAR  = 0x00000000u, //!< Linear memory layout.
	SCE_GXM_DEPTH_STENCIL_SURFACE_TILED   = 0x00011000u  //!< Tiled memory layout.
...
typedef enum SceGxmMultisampleMode {
	SCE_GXM_MULTISAMPLE_NONE, //!< Multisample disabled.
	SCE_GXM_MULTISAMPLE_2X,   //!< 2x1 rotated grid multisample.
	SCE_GXM_MULTISAMPLE_4X    //!< 2x2 rotated grid multisample.
...
typedef enum SceGxmTextureBaseFormat {
	...
	SCE_GXM_TEXTURE_BASE_FORMAT_PVRT2BPP     = 0x80000000, //!< PowerVR Texture Compression (PVRTC 2-bpp).
	SCE_GXM_TEXTURE_BASE_FORMAT_PVRT4BPP     = 0x81000000, //!< PowerVR Texture Compression (PVRTC 4-bpp).
	SCE_GXM_TEXTURE_BASE_FORMAT_PVRTII2BPP   = 0x82000000, //!< PowerVR Texture Compression (PVRTC2 2-bpp).
	SCE_GXM_TEXTURE_BASE_FORMAT_PVRTII4BPP   = 0x83000000, //!< PowerVR Texture Compression (PVRTC2 4-bpp).
	SCE_GXM_TEXTURE_BASE_FORMAT_ETC1         = 0x84000000, //!< Ericsson Texture Compression (ETC1). Requires SCE_GXM_INITIALIZE_FLAG_EXTENDED_FORMAT
	SCE_GXM_TEXTURE_BASE_FORMAT_UBC1         = 0x85000000, //!< Unsigned BC1 (DXT1).
	SCE_GXM_TEXTURE_BASE_FORMAT_UBC2         = 0x86000000, //!< Unsigned BC2 (DXT2/DXT3).
	SCE_GXM_TEXTURE_BASE_FORMAT_UBC3         = 0x87000000, //!< Unsigned BC3 (DXT4/DXT5).
	SCE_GXM_TEXTURE_BASE_FORMAT_UBC4         = 0x88000000, //!< Unsigned BC4.
	SCE_GXM_TEXTURE_BASE_FORMAT_SBC4         = 0x89000000, //!< Signed BC4.
	SCE_GXM_TEXTURE_BASE_FORMAT_UBC5         = 0x8A000000, //!< Unsigned BC5.
	SCE_GXM_TEXTURE_BASE_FORMAT_SBC5         = 0x8B000000, //!< Signed BC5.
...
typedef enum SceGxmTextureType {
	SCE_GXM_TEXTURE_SWIZZLED             = 0x00000000u, //!< Swizzled memory layout with power of two width and height
	SCE_GXM_TEXTURE_CUBE                 = 0x40000000u, //!< Cube memory layout with power of two width and height
	SCE_GXM_TEXTURE_LINEAR               = 0x60000000u, //!< Linear memory layout
	SCE_GXM_TEXTURE_TILED                = 0x80000000u, //!< Tiled memory layout
	SCE_GXM_TEXTURE_SWIZZLED_ARBITRARY   = 0xA0000000u, //!< Swizzled memory layout with arbitrary width and height
	SCE_GXM_TEXTURE_LINEAR_STRIDED       = 0xC0000000u, //!< Linear memory layout with arbitrary stride
	SCE_GXM_TEXTURE_CUBE_ARBITRARY       = 0xE0000000u  //!< Cube memory layout with arbitrary width and height.
} SceGxmTextureType;
...
typedef enum SceGxmProgramType {
	SCE_GXM_VERTEX_PROGRAM,   //!< Vertex shader program
	SCE_GXM_FRAGMENT_PROGRAM  //!< Fragment shader program
} SceGxmProgramType;
